...
Material &MaterialBroker::retrieveMaterial(std::string materialId)
{
    ...
    boost::json::object jsonMaterial = RelationalBroker::retrieveMaterialRelation(command);

    std::string id, path, jId;
    int type;
    id = jsonMaterial["id"].as_string();
    path = jsonMaterial["path"].as_string();
    type = jsonMaterial["type"].as_uint64();
    jId = jsonMaterial["jId"].as_string();

    Material material(id, jId, path, type);
...
void MaterialBroker::newCacheFlush()
{
    for (auto iter = m_newClean.begin(); iter != m_newClean.end();) {
        ...
        //将数据插入数据库
        std::string command = "insert into Material (M_id,M_path,J_id,M_type) values(" + iter->first
                              + ",'" + iter->second.path() + "'," + iter->second.jottingId() + ","
                              + iter->second.type() + ")";
...
MaterialBroker::~MaterialBroker()
{
    ...
    for (auto &material : m_newClean) {
        std::string command = "insert into Material (M_id,M_path,J_id) values("
                              + material.second.id() + "," + material.second.path() + ","
                              + material.second.jottingId() + ")";
        RelationalBroker::insert(command);
    }
}
